...
class M_posts extends CI_Model {
	...
	public function __construct() {
		...
		$this->postPerTagCat = postPerTagCat();
	}
	...
	public function get_where($keyword = '', $limit = 0, $offset = 0) {
		...
		if (in_array($this->session->userdata('user_type'), ['student', 'employee'])) {
			$this->db->where('x1.post_author', (int) $this->session->userdata('id'));
		}
		if (!empty($keyword)) {
			$this->db->group_start();
	...
	public function more_posts($slug = '', $offset = 0) {
		...
		$this->db->order_by('x1.created_at', 'ASC');
		return $this->db->get(self::$table.' x1');
	}
	...
	public function more_posts_by_tag($tag = '', $offset = 0) {
		...
		$this->db->group_end();
		if ($offset < 0) {
			$this->db->limit($this->postPerTagCat);
		}
		if ($offset > 0) {
			$this->db->limit($this->postPerTagCat, $offset);
		}
		return $this->db->get(self::$table.' x1');
	}
	 ...
	 * @return query
	 */
	public function latest_posts($limit = 10) {
		$this->db->select('x1.id
			, x1.post_title
			, x2.user_full_name AS post_author
			, x1.post_content
			, x1.created_at
